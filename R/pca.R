...
#' Alternatively, a character assay name from \code{data}.
...
#' @rdname pca
#' @export
pca <- function(X=NULL, ncomp=2, ...) {
   UseMethod('pca')
}

#' @param data An object of class \code{MultiAssayExperiment},
#' \code{SummarizedExperiment}, or \code{SingleCellExperiment}.
#' @importFrom SummarizedExperiment assay assays
#' @rdname pca
#' @export
pca.character <- function(X=NULL, ncomp=2, ..., data=NULL) {
   mc <- match.call()
   vdc <- c("MultiAssayExperiment", ## valid data classes
            "SummarizedExperiment",
            "SingleCellExperiment")
   .check_data_assay(mc, valid_classes = vdc ) ## check data and assay sanity
   mc[-1] <- lapply(mc[-1], eval.parent)   ## evaluate the args
   mcr <- mc ## copy to return as call before changing it
   mc$X <- t(assay(data, X)) ## change X into the assay matrix
   mc$data <- NULL ## remove data as not needed in internal
   mc[[1L]] <- quote(.pca) ## change the call to internal
   result <- eval(mc) ## evaluate the call
   result$call <- mcr ## chnage the returned call to current call
   return(result)
}
